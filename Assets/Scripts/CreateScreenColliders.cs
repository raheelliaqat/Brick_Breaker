...
public class CreateScreenColliders : MonoBehaviour
{
...

    {
        ...
        DefaultWidth = Camera.main.orthographicSize * 0.625f;
        ...
        colliders["Top"].position = new Vector3(cameraPos.x, cameraPos.y + screenSize.y + (colliders["Top"].localScale.y * -1.5f), zPosition);
        //colliders["Bottom"].position = new Vector3(cameraPos.x, cameraPos.y - screenSize.y - (colliders["Bottom"].localScale.y * 0.5f), zPosition);

        rse = cameraPos.x + (screenSize.x + (colliders["Right"].localScale.x * 0.5f)) - 2.65f;
        lse = (cameraPos.x - screenSize.x - (colliders["Left"].localScale.x * 0.5f)) + 2.65f;

        //Debug.Log("Left Screen Edge: " + lse);
       // Debug.Log("Right Screen Edge: " + rse);
        for (float x =-5.5f; x<=5.7f; x+= 1.6f)
        {
            for(float y=3.2f; y>= -1.7f; y-=0.7f)
            {
                randprefab = Random.Range(0, 4);
                Instantiate(prefabs[randprefab], new Vector3(x, y), Quaternion.identity);
            }
        }
